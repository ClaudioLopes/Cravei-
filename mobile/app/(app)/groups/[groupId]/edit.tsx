...
import { Button, Card, EmptyState, ErrorText, Field, Screen, Subtitle, Title } from '../../../../src/components/ui';
...
export default function EditGroupScreen() {
  ...
  useEffect(() => {
    ...
    setNome(group.nome);
    setTipo(group.tipo);
    setScoringTipo(group.scoringConfig.tipo);
    setPontosPlacarExato(String(group.scoringConfig.pontosPlacarExato ?? 3));
    setPontosResultado(String(group.scoringConfig.pontosResultado ?? 1));
    setPodioTamanho(group.podioTamanho === 3 ? 3 : 1);
  }, [group]);
    ...
    mutationFn: async () => {
      const scoringConfig: ScoringConfig =
        scoringTipo === 'camadas'
          ...
          : { tipo: scoringTipo };
      return updateGroup(groupId, { nome: nome.trim(), tipo, scoringConfig, podioTamanho });
    },
    onSuccess: async () => {
  ...
  if (isLoading || !group) {
    return (
      <Screen>
        <Stack.Screen options={{ title: 'Editar grupo' }} />
      </Screen>
    );
  }

  if (group.donoId !== user?.id) {
    return (
      <Screen>
        <Stack.Screen options={{ title: 'Editar grupo' }} />
        <EmptyState message="Apenas o criador do grupo pode editá-lo." />
      </Screen>
    );
  }
  ...
  if (group.iniciado) {
    return (
      <Screen>
        <Stack.Screen options={{ title: 'Editar grupo' }} />
        <EmptyState message="Este grupo já teve palpites ou turnos iniciados e não pode mais ser editado." />
      </Screen>
    );
  }
  ...
  return (
    <Screen>
      ...
      <Subtitle>A regra trava assim que alguém enviar o primeiro palpite do turno.</Subtitle>
